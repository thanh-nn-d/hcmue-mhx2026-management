...
function AdminDashboard() {
  ...
  const { activities, updateFeedback, markAdminSeen } = useActivities();
  ...
  const showAssignedPointFilter =
    teamFilter === "" || teamConfig[teamFilter]?.hasAssignedPoint;
  ...
  const handleTeamChange = (e) => {
    ...
    if (value !== "" && !teamConfig[value]?.hasAssignedPoint) {
      setPointFilter("");
    }
  };
  ...
  const teamActivities =
    teamFilter === ""
      ? activities
      : activities.filter((activity) => activity.team === teamFilter);

  const filteredActivities = useActivityFilter({
    activities: teamActivities,
    searchKeyword,
    statusFilter,
    pointFilter: showAssignedPointFilter ? pointFilter : "",
    sortType,
  });

  const newActivities = activities.filter(
    (activity) => activity.adminSeen === false
  );
  ...
  const handleSaveFeedback = (activityId, feedback) => {
      ...
      <main className="mx-auto max-w-7xl px-8 pb-10 pt-5">
        ...
        {newActivities.length > 0 && (
          <div
            ...
            className="mb-6 mt-6 cursor-pointer rounded-2xl border border-blue-200 bg-blue-50/95 p-5 shadow-lg transition hover:bg-blue-100"
          >
            <p className="font-bold text-blue-800">
              🔔 Có{" "}
              <span className="font-black">{newActivities.length}</span>{" "}
        ...
        <div className="rounded-2xl border border-white/60 bg-white/90 p-5 shadow-lg backdrop-blur">
          ...
          <ActivityTable
            ...
            onDelete={() => {}}
            showTeamColumn={true}
            showAssignedPointColumn={showAssignedPointFilter}
            showEdit={false}
            showDelete={false}
          />
        </div>
